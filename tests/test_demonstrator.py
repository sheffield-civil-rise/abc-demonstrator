...
class TestDemonstrator:
    ...
    def __init__(self):
        ...
        self.demo_obj.make_and_run_reconstruction_dir_generator()

    def test_rec_dir_gen_fields(self):
        """ Test the FIELDS of the ReconstructionDirGenerator sub-object. """
        assert (
            self.demo_obj.rec_dir_gen.path_to_output ==
                config.DEFAULT_PATH_TO_DEMO_OUTPUT
        )
        assert (
            self.demo_obj.rec_dir_gen.path_to_output_images ==
                os.path.join(config.DEFAULT_PATH_TO_DEMO_OUTPUT, "images")
        )
        assert (
            self.demo_obj.rec_dir_gen.path_to_labelled_images ==
                os.path.join(config.DEFAULT_PATH_TO_DEMO_OUTPUT, "labelled")
        )
        assert (
            self.demo_obj.rec_dir_gen.path_to_masked_images ==
                os.path.join(config.DEFAULT_PATH_TO_DEMO_OUTPUT, "masked")
        )

    def test_rec_dir_gen_files(self):
        """ Test that the ReconstructionDirGenerator sub-object outputs the
        right FILES. """
        ...
        actual_camera_init_checksum = \
            make_checksum(
                os.path.join(
                    self.demo_obj.rec_dir_gen.path_to_output, "cameraInit.sfm"
                )
            )
        actual_camera_init_label_checksum = \
            make_checksum(
                os.path.join(
                    self.demo_obj.rec_dir_gen.path_to_output, "cameraInit_label.sfm"
                )
            )
